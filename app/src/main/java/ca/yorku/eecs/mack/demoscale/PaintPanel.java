...
public class PaintPanel extends View
{
    ...
    private void initialize(Context context)
    {
        ...
        targetImage = context.getResources().getDrawable(R.drawable.a1);
    ...
    private class MyGestureListener extends GestureDetector.SimpleOnGestureListener
    {
        ...
        public boolean onDoubleTap(MotionEvent me)
        {
            ...
            RectF r = new RectF(left, top, right, bottom);
            boolean inside = r.contains(x, y);
            //if touch piont is inside the image then scale it
            if(!inside)return false;
            //scale the pic as the center of the click
            if(scaleFactor==0.5f) {
               scaleFactor=1.5f;
               // ensure the scaling occurs about the focus point
               float focusX = me.getX();
               float focusY = me.getY();
               xPosition = focusX-imageIntrinsicWidth * scaleFactor*xRatio;
               yPosition = focusY-imageIntrinsicHeight * scaleFactor*yRatio;
               return true;
            }
            if(scaleFactor==1.5f){
                scaleFactor=0.5f;
